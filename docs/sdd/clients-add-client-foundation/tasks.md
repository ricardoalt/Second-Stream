...
- [x] Add `account_status` column to the backend `Company` model. It MUST be an enum or constrained string allowing only `lead` or `active`.
- [x] Generate and run the database migration. Ensure the migration aligns legacy values and defaults with lifecycle-managed `lead` creation (transition to `active` is backend lifecycle logic, not Add Client input).
...
- [x] Update the `onSubmit` handler to perform the sequential creation workflow:
  ...
  - If Company succeeds, call `createCompanyContact` with `isPrimary: true`. If it fails, route to `/leads/{companyId}?create=partial-contact`.
  - If Contact succeeds, call `createLocation` with `addressType: 'headquarters'`. Explicitly avoid inferring `addressType` from `customerType`. If it fails, route to `/leads/{companyId}?create=partial-location`.
  - If all succeed, route to `/leads/{companyId}?create=success`.

## 4. Client Profile Banners (Active UI Mount)
- [x] Modify `frontend/app/(agent)/leads/[id]/page.tsx` (or its layout) to read the `?create=` search parameter.
...
- [x] Verify that Add Client creation persists `account_status` as `lead` (and that only `lead|active` are allowed by API/domain constraints).
