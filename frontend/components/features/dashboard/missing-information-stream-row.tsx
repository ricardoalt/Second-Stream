import { ChevronDown, Mail, AlertCircle, Clock, CheckCircle2 } from "lucide-react";
			...
			>
				<div className="flex items-center gap-4">
					<div className={cn("size-10 rounded-full flex items-center justify-center shrink-0", 
						stream.priority === "critical" ? "bg-destructive/10 text-destructive" : 
						stream.priority === "high" ? "bg-warning/10 text-warning-foreground" : "bg-primary/10 text-primary"
					)}>
				...
				<div className="flex flex-wrap items-center gap-2">
					...
					{stream.missingItems.slice(0, 2).map(item => (
						<div key={item} className="rounded border border-border bg-surface-container-low px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wider text-muted-foreground whitespace-nowrap overflow-hidden text-ellipsis max-w-[100px]">
							{item}
						</div>
					))}
				...
				<div className="flex flex-col items-center">
					<div className="flex w-full justify-between items-end mb-1.5 px-1">
						<span className={cn("text-[10px] font-bold uppercase tracking-wider", 
							stream.priority === "critical" ? "text-destructive" :
							stream.priority === "high" ? "text-warning-foreground" : "text-muted-foreground"
						)}>
							{stream.statusLabel}
						</span>
					</div>
				...
				<div className="flex items-center justify-end gap-6">
					...
					<Button size="icon" variant="outline" className="size-8 rounded-lg bg-primary text-primary-foreground border-none hover:bg-primary/90">
