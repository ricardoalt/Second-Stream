...
type CompanyContactsPageContentProps = {
	...
	loadCompany: (companyId: string) => Promise<CompanyDetail>;
	loadLocationSummaries: (companyId: string) => Promise<{ id: string }[]>;
	loadLocationDetail: (locationId: string) => Promise<LocationDetail>;
};

export function CompanyContactsPageContent({
	companyId,
	lifecycle,
	loadCompany,
	loadLocationSummaries,
	loadLocationDetail,
}: CompanyContactsPageContentProps) {
	...
	const loadContactsHub = useCallback(async () => {
		...
		setError(null);
		try {
			const companyDetail = await loadCompany(companyId);
			const locationSummaries = await loadLocationSummaries(companyId);
			const locationDetails = await Promise.all(
				locationSummaries.map((location) => loadLocationDetail(location.id)),
			);
	...
	}, [companyId, loadCompany, loadLocationDetail, loadLocationSummaries]);
								...
								{locations.length === 0 ? (
									<div className="flex items-center justify-center rounded-xl border border-dashed py-12 px-6 text-center">
										<p className="text-sm text-muted-foreground">
											No active locations found for this company.
										</p>
									</div>
								) : (
