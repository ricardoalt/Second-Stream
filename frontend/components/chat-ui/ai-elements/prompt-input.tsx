...
import {
	...
	AttachmentPreview as AttachmentItemPreview,
	AttachmentRemove as AttachmentItemRemove,
	Attachments as AttachmentsContainer,
} from "./attachments";
...
export type PromptInputAttachmentsProps = Omit<
	ComponentProps<typeof AttachmentsContainer>,
	"children"
> & {
...
export const PromptInputAttachments = ({
	...
	return (
		<AttachmentsContainer variant={variant} {...props}>
			{attachments.files.map((attachment) => (
				<Fragment key={attachment.id}>{children(attachment)}</Fragment>
			))}
...
export type PromptInputAttachmentProps = Omit<
	ComponentProps<typeof AttachmentItem>,
	"data" | "onRemove"
> & {
...
export const PromptInputAttachment = ({
	...
	}, [attachments, data.id, onRemove]);

	return <AttachmentItem data={data} onRemove={handleRemove} {...props} />;
};

export type PromptInputAttachmentPreviewProps = ComponentProps<
	typeof AttachmentItemPreview
>;
...
> & {
	...
	maxFileSize?: number;
	onError?: (err: {
		code: "max_files" | "max_file_size" | "accept" | "read_failed";
		message: string;
	}) => void;
	onSubmit: (
		...
		async (event) => {
			...
			try {
				...
				const convertedFiles: FileUIPart[] = await Promise.all(
					files.map(async ({ id: _id, ...item }) => {
						if (item.url?.startsWith("blob:")) {
							...
							if (!dataUrl) {
								throw new Error("We couldn't read one or more files.");
							}
				...
				onError?.({
					code: "read_failed",
					message:
						"We couldn't read one or more files. Remove them and try again.",
				});
				// Don't clear on error - user may want to retry
			}
		},
